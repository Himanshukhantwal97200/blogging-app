...
class _EmbedButtonState extends State<EmbedButton> {
  ...
  Widget buildOverlay(BuildContext context) {
    ...
    final buttons = Row(
      children: <Widget>[
        SizedBox(width: 8.0),
        toolbar.buildButton(context, ZefyrToolbarAction.videoUrl,
            onPressed: showGetUrlDialog),
      ],
    );
    return ZefyrToolbarScaffold(body: buttons);
  }
        ...
        return AlertDialog(
          ...
          content: TextField(
            ...
            decoration: InputDecoration(
                labelText: "آدرس اینترنتی ویدئو (فرمت فیلم .mp4 باشد)"),
          ),
          actions: <Widget>[
            FlatButton(
              child: Text("افزودن به متن"),
              onPressed: () {
                if (videoSearchText.contains("https://") ||
                    videoSearchText.contains("http://") &&
                        videoSearchText.contains('mp4')) {
                  editor.formatSelection(
                      NotusAttribute.embed.video(videoSearchText));
